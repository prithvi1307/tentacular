...
`tntc init` is the single highest-priority item on this roadmap. Everything else — the catalog, the UX refactor, the example workflows story — depends on it. Without a scaffold command, users had to copy workflows by hand, the skill documented a workaround instead of a feature, and the onboarding story was broken. **Status: Resolved.** Both `tntc init` and `tntc catalog init` are now available. Workflow templates live in [tentacular-catalog](https://github.com/randybias/tentacular-catalog).
...
- Scaffold content is embedded Go strings (`//go:embed`), not read from external sources
- No network calls — fully offline

---

**What it unlocks:**
- `example-workflows/` removed from the core repo (replaced by tentacular-catalog)
- The skill documents `tntc init` for blank scaffolds and `tntc catalog init` for templates
- The catalog story is now live: `tntc catalog list`, `tntc catalog init <template>`
...
The tool itself: Go CLI (`cmd/`, `pkg/`) and Deno engine (`engine/`). Versioned releases. Workflow templates live in `tentacular-catalog`, not the tool repo.
...
The default source for `tntc catalog init`. Seeded from the original `example-workflows/` directory. Each workflow is a subdirectory with a `template.yaml` metadata file; community contributions are PRs to this repo without touching the CLI. The catalog index (`catalog.yaml`) is auto-generated by CI.
...
**Relationship to the skill:** Workflow templates live in `tentacular-catalog`. The skill documents `tntc catalog init` for template-based scaffolding.
...
**Resolved Feb 2026.** All workflows consolidated, then migrated to `tentacular-catalog` repo (Mar 2026). AGENTS.md, README.md, and architecture.md updated. Multi-arch build convention (`docker buildx`) added.
